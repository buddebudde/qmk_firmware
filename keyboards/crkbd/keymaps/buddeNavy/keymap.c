...
uint32_t layer_state_set_user(uint32_t state) {
    ...
    case _LOWER:
        if(isLeftHand) {
          ...
          rgblight_setrgb_at(0, 0, 255, 13); // sets LED number 13 (lower) to purple on left hand

        } else {

          for(int i=10; i < 21; i ++) {
            if(i != 13){
              rgblight_setrgb_at(0, 0, 255, i); // sets LED 10-21 but not 13 to cyan on right hand
            }
          }
        }
        break;
